...
| `cross_tenant_access.json` | `{}` | Cross-tenant access policy: the default configuration plus per-partner overrides. A partner's `null` fields inherit the default. | `default` (`inboundTrust.isMfaAccepted` and device-trust flags, `automaticUserConsentSettings`, b2b collaboration/direct-connect blocks), `partners[]` (`tenantId`, same fields; `inboundTrust: null` means inherited). Either half `null` means unreadable, not unconfigured. |
